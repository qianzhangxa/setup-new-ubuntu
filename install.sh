...
echo -e "\n===== Comment out 'secure_path' in /etc/sudoers =====\n"
sudo sed -i 's|^\(Defaults[[:space:]]*secure_path=.*\)|# \1|' /etc/sudoers
...
echo -e "\n===== Install bat, net tools and build tools =====\n"
sudo apt install -y bat net-tools build-essential gdb
